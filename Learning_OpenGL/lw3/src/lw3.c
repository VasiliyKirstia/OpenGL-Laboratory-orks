...
const float rotation_delta = 5.0f;
...
void display(void){
	...
	glRotatef(Settings.rotation_speed, Settings.XYZ & 1, Settings.XYZ & 2, Settings.XYZ & 4);
...
void keyboard(unsigned char key, int x, int y){
	switch(key){
		...
		break;
		case 43:
			Settings.rotation_speed += rotation_delta;
		break;
		case 45:
			Settings.rotation_speed -= rotation_delta;
		break;
